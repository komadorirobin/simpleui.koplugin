...
--
-- Public API used by main.lua / sui_patches.lua / module_book_rows.lua:
...
-- TBR-specific arrange menu: uses isTBR/removeTBR, not a generic collection.
    ...
    return {
        {
            ...
            sub_item_table_func = function()
                    ...
                    callback       = function()
                        ...
                        if #list < 2 then
                            _UIManager:show(InfoMessage:new{
                                text = _lc("Add at least 2 books to arrange."),
                                timeout = 2,
                            })
                            return
                        end
                ...
                if #list == 0 then
                    sub_items[#sub_items + 1] = {
                        text = _lc("No books in To Be Read list."),
                        enabled = false,
                    }
                else
                    for _, fp in ipairs(list) do
...
-- module_collections.lua), while consolidating all book row module
