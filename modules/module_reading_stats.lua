...
local StreakFreeze = require("infra/sui_streak")
...
-- ---------------------------------------------------------------------------
-- Card text size, as a percentage of the card's own width (mirrors
-- module_clock.lua / module_coverdeck.lua's convention: 100% scale = sized
-- to what's actually available, not a fixed physical/point constant).
--
-- Value/label text follows the card's real width: narrower when more stat
-- cards are selected (up to RS_N_COLS) or the row itself is narrower
-- (landscape spread — `w` is already narrowed upstream, see
-- sui_screen_engine.lua), wider when fewer cards are shown. Growth is capped
-- relative to card_h (see val_fs_max/lbl_fs_max in build()) so text never
-- outgrows the card when very few cards are selected.
--
-- The percentage is derived once at module load from a reference card: a
...
local _RS_VAL_FS_MIN   = 8
local _RS_LBL_FS_MIN   = 6

local function _clampFs(v, lo, hi)
...
local STAT_MAP = {
    ...
    -- why they'd differ.
    status_unread    = { display_label = _("Status — Unread"),    value = function(s) return tostring(s.status_unread) end,    label = _("unread") },
    status_reading   = { display_label = _("Status — Reading"),   value = function(s) return tostring(s.status_reading) end,   label = _("in progress") },
    status_abandoned = { display_label = _("Status — Abandoned"), value = function(s) return tostring(s.status_abandoned) end, label = _("abandoned") },
}
...
local STAT_POOL = { "today_time","today_pages","week_time","week_pages","avg_time","avg_pages","month_time","month_pages","total_time","total_books","streak",
                     "status_unread","status_reading","status_abandoned" }

-- Stat ids backed by SP.getStatusCounts() (a whole-library filesystem walk)
-- instead of the DB/ReadHistory-backed fields above. Kept as its own set so
-- callers can cheaply check "is a status_* card selected" without a table
-- walk of STAT_MAP itself.
local STATUS_STAT_IDS = { status_unread = true, status_reading = true, status_abandoned = true }

--- True if at least one of the currently-selected stat cards needs
--- SP.getStatusCounts(). Used by the engine to gate that walk the same way
--- "total_books" already gates the ReadHistory sidecar scan.
local function _usesStatusCounts(pfx)
    for _, id in ipairs(_getItems(pfx)) do
        if STATUS_STAT_IDS[id] then return true end
    end
    return false
end

-- Maps each stat_id to the _changed category it belongs to (from SP.get()),
-- so updateStats() can skip rebuilding cards whose data did not change.
-- Categories: "timeseries" (fetchTimeSeries fields), "streak" (fetchStreak),
-- "books" (sidecar scan). Ids not listed here (status_*) are always rebuilt.
...
--- success so the caller can fall back to openReadingInsights() when
--- sui_stats_windows is unavailable.
...
--- Opens the detail screen for a tapped stat card, if it has one
--- (total_books -> finished-books dialog, streak -> Streak Manager).
--- Shared by both tap handlers in M.build() below. Returns true if a
--- detail screen was opened.
local function _openStatDetail(stat_ids, idx, n)
    if idx < 1 or idx > n then return false end
    local sid = stat_ids[idx]
    if sid == "total_books" then
        local ok, SW = pcall(require, "screens/sui_stats_windows")
        if ok and SW and SW.showFinishedBooksDialog then
            SW.showFinishedBooksDialog()
            return true
        end
    elseif sid == "streak" then
        return openStreakManager()
    end
    return false
end
...
-- Public accessor for the selected stat-card ids, applying the default
-- { "total_books", "today_time", "streak" } set. Callers outside this file
-- (e.g. sui_homescreen's needs_books check) must use this rather than
-- reading "reading_stats_items" directly: a raw nil/empty read on a fresh
-- install means "use the defaults", not "no items".
M.getItems = _getItems
M.usesStatusCounts = _usesStatusCounts
...
function M.build(w, ctx)
    ...
    --
    -- Two scale bases, same split as module_clock.lua's build(): `scale`
    -- (module scale × lf) drives fixed-pixel dimensions (card height, gaps,
    -- separators). `raw_scale` (module scale only) drives card text size,
    -- since that's derived from item_w, which is already narrowed for
    -- landscape via `w` upstream — applying lf again would narrow it twice.
    ...
    -- Per-card width used as the text-sizing basis. Mirrors the "cards"
    -- layout's own card_w formula (below) regardless of the selected
    ...
    local stats = {
        ...
        status_unread    = (ctx.status_counts and ctx.status_counts.unread)    or 0,
        status_reading   = (ctx.status_counts and ctx.status_counts.reading)   or 0,
        status_abandoned = (ctx.status_counts and ctx.status_counts.abandoned) or 0,
    }
    ...
    if mode == "list" then
        ...
        function tappable:onTapStatCard(_, ges)
            if ges and ges.pos and self.dimen then
                ...
                if _openStatDetail(stat_ids, idx, n) then return true end
            end
            openReadingInsights()
            return true
        end
        ...
        -- GestureRange + one handler replaces N of each (N ≤ RS_N_COLS).
        ...
        function tappable:onTapStatCard(_, ges)
            if ges and ges.pos and self.dimen then
                ...
                if dx >= 0 then
                    ...
                    if (dx % step) <= card_w and _openStatDetail(stat_ids, idx, n) then
                        return true
                    end
                end
            end
...
function M.getHeight(_ctx)
    ...
    local card_h = math.floor(_BASE_RS_CARD_H * Config.getModuleScale("reading_stats", _ctx and _ctx.pfx) * lf)
    return card_h
end
