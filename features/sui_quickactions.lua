...
local function _Bottombar()
    return package.loaded["screens/sui_bottombar"] or require("screens/sui_bottombar")
end
...
-- Exported so other modules (e.g. sui_bottombar's floating bar window) can
-- reuse this resolution instead of duplicating it — same convention as the
-- QA.trackIndicatorWhileOpen/ViaCallback helpers below.
QA.resolveSimpleUIPlugin = _resolveSimpleUIPlugin
...
local function _goHome(target_fm)
    ...
    -- isInSeriesView/exitSeriesView live in sui_series_grouping.lua, NOT
    -- sui_foldercovers.lua (fixed bug: this used to require the wrong
    -- module, so in_virtual was always false and a series-group view
    -- sitting directly in the home dir fell into the onGotoPage(1) branch
    -- below instead of changeToPath(home) — the titlebar path got updated
    -- but the FileChooser's item_table, still the series listing, never
    -- was, so the visible page appeared frozen).
    local ok_sg_mod, SG_mod = pcall(require, "features/library/sui_series_grouping")
    local in_virtual = ok_sg_mod and SG_mod.isInSeriesView and SG_mod.isInSeriesView(fc)
    -- No need to call SG_mod.exitSeriesView(fc) here: it would just do its
    -- own changeToPath(parent), immediately overridden by the
    -- changeToPath(home) below — changeToPath() already exits series view
    -- on its own via the patched switchItemTable (sui_series_grouping.lua).
...
local function _doWifiToggle(plugin)
    ...
    if plugin then
        ...
        -- 3. Quick-action icons — baked into ImageWidgets, need a full
        -- rebuild. Whichever screen is actually open (built-in Homescreen or
        -- a Custom Screen) can host these, so reach whatever's live rather
        -- than assuming the Homescreen.
        local ScreenEngine = package.loaded["engines/sui_screen_engine"]
        if ScreenEngine then
            pcall(function() ScreenEngine.refreshAllLiveImmediate(false) end)
        end
    end
...
local function _showBookmarkBrowserSourceDialog(bb_ui)
    ...
    -- Whichever screen (built-in Homescreen or a Custom Screen) is open
    -- right now, if any — generic via UI.getOpenScreen() (infra/sui_core.lua
    -- — single authority also used by sui_bottombar.lua's M.navigate())
                ...
                for i = #stack, 1, -1 do
                    ...
                    if w and w.covers_fullscreen and w.name ~= "homescreen" then
                        ...
                        w.onCloseWidget = function(self_w)
                            if open_screen_inst then
                                open_screen_inst._navbar_closing_intentionally = true
                                UIManager:close(open_screen_inst)
                            end
                            if plugin then
    ...
    source_dialog = ButtonDialog:new{
        ...
        buttons = {
            ...
            {{ text = _("Cancel"), callback = function()
                ...
                UIManager:close(source_dialog)
                if hs_was_open and open_screen_inst then
                    UIManager:setDirty(open_screen_inst, "full")
                end
            end }},
        },
    }
        ...
        },
        {
            ...
            execute = function(ctx)
                ...
                QA.trackIndicatorViaCallback(plugin, "sui_settings", function(restore)
                    require("screens/sui_settings_window"):show(restore)
                end)
            end,
        },
        {
...
-- Returns true when `id` is known to the registry — built-in OR externally
-- registered via QA.register() (e.g. a Custom Screen's
-- "open_custom_screen:<id>" QA, see infra/sui_custom_screens.lua).
-- Single source of truth for "is this a real action id", used both inside
-- this module (QA.sanitizeQASlots' prune passes) and by infra/sui_config.lua's
-- loadTabConfig() (via a lazy require, to avoid a load-order cycle — Config
-- is required at this file's own top level).
function QA.isRegistered(id)
    return id ~= nil and _registry[id] ~= nil
end
...
function QA.sanitizeQASlots()
    ...
    for _i, id in ipairs(list) do
        ...
        if type(cfg) == "table" and cfg.qa_folder then
            ...
            for _j, member_id in ipairs(items) do
                -- QA.isRegistered, not just Config.ACTION_BY_ID: a Custom
                -- Screen's "open_custom_screen:<id>" QA (or any other
                -- externally registered action) is neither a built-in nor a
                -- custom_qa_%d+, so it was being silently pruned from every
                -- group it belonged to on every plugin boot.
                local member_valid = Config.ACTION_BY_ID[member_id] or QA.isRegistered(member_id)
    ...
    if type(tabs) == "table" then
        ...
        for _i, id in ipairs(tabs) do
            -- Same fix as the group-member pruning above: QA.isRegistered
            -- covers externally registered actions (Custom Screens) that
            -- Config.ACTION_BY_ID never will, since Config.ALL_ACTIONS only
            -- mirrors the built-ins.
            if Config.ACTION_BY_ID[id] or QA.isRegistered(id)
...
function QA.performResetAllQAIcons(plugin)
    ...
    if ok_cs and CustomScreens then
        for _, screen in ipairs(CustomScreens.list()) do
            CustomScreens.setIcon(screen.id, nil)
        end
    end
    local ok_ss, SUIStyle = pcall(require, "features/sui_style")
    ...
    -- Whichever screen is live (Homescreen or Custom Screen) gets the
    -- immediate repaint, not just the Homescreen.
    local ok, ScreenEngine = pcall(require, "engines/sui_screen_engine")
    if ok and ScreenEngine then ScreenEngine.refreshAllLiveImmediate(false) end
end

-- ---------------------------------------------------------------------------
-- sui_show_qa_list(plugin, ctx_menu, ctx) — top-level "Quick Actions" screen
-- ---------------------------------------------------------------------------
-- Two entries only, same shape as any other main submenu (Style, Bars, ...):
-- no subtitles here, just text (+ the "✎" customized-state indicator on
-- Quick Action Icons, same convention as Style → Icons) and a chevron.
function QA.sui_show_qa_list(plugin, ctx_menu, ctx)
    local function get_rows()
        return {
            {
                text         = ctx_menu._("Quick Actions Icons") .. (QA.hasCustomIcons() and "  \u{270E}" or ""),
                show_chevron = true,
                on_tap       = function()
                    QA.sui_build_qa_icons(plugin, ctx_menu, ctx)
                end,
            ...
            {
                text         = ctx_menu._("Custom Quick Actions"),
                right_value  = string.format("%d/%d", #Config.getCustomQAList(), Config.MAX_CUSTOM_QA),
                show_chevron = true,
                on_tap       = function()
                    QA.sui_show_custom_qa_list(plugin, ctx_menu, ctx)
                end,
            },
        }
    end
...
-- ---------------------------------------------------------------------------
-- sui_show_custom_qa_list(plugin, ctx_menu, ctx) — "Custom Quick Actions"
-- sub-screen: the user's custom quick actions (formerly shown at the root
-- of the Quick Actions screen, alongside the pinned Icons row) plus the
-- "Create Quick Action" footer, now that creating one belongs here rather
-- than on the two-entry parent screen.
-- ---------------------------------------------------------------------------
function QA.sui_show_custom_qa_list(plugin, ctx_menu, ctx)
    local function get_rows()
        local rows = {}
    ...
    ctx_menu.show_row_page({
        title = ctx_menu._("Custom Quick Actions"),
        items_func = get_rows,
...
function QA.sui_build_qa_icons(plugin, ctx_menu, ctx)
    ...
    -- Every Custom Screen (infra/sui_custom_screens.lua) auto-registers its
    -- own "open_custom_screen:<id>" Quick Action, but that registration
    -- lives in the external QA registry, not in Config.ALL_ACTIONS or the
    -- custom-QA list above — so it needs its own pool entries here to be
    -- reachable from this screen at all. Lazy require: sui_custom_screens
    -- lazily requires this file back for QA.register/unregister.
    local ok_cs, CustomScreens = pcall(require, "infra/sui_custom_screens")
    if ok_cs and CustomScreens then
        for _, screen in ipairs(CustomScreens.list()) do
            pool[#pool + 1] = {
                id               = CustomScreens.qaId(screen.id),
                is_default       = false,
                is_custom_screen = true,
                screen_id        = screen.id,
                title            = screen.name,
            }
        end
    ...
    local function get_rows()
        ...
        for _, entry in ipairs(pool) do
            ...
            elseif entry.is_custom_screen then table.insert(screens, entry)
        ...
        local function add_entry(entry)
            ...
            local _is_screen  = entry.is_custom_screen
            local _screen_id  = entry.screen_id
            ...
            elseif _is_screen then
                local screen = ok_cs and CustomScreens.get(_screen_id)
                current_icon = screen and screen.icon
            else
                current_icon = Config.getCustomQAConfig(_id).icon
            end
            local has_custom
            if _is_default then
                has_custom = QA.getDefaultActionIcon(_id) ~= nil
            elseif _is_screen then
                has_custom = current_icon ~= nil
            else
                local c = Config.getCustomQAConfig(_id)
                has_custom = c.icon ~= nil
            ...
            if not effective_icon then
                ...
                elseif _is_screen then
                    ...
                    -- Config.ICON.custom), so the preview matches what the
                    -- Quick Action actually shows today.
                    effective_icon = Config.ICON.custom
                else
                    local c = Config.getCustomQAConfig(_id)
                    if c.qa_folder then
                ...
                on_tap = function()
                    ...
                    QA.showIconPicker(current_icon, function(new_icon)
                        ...
                        if Config.isNerdIcon(new_icon) then
                            ...
                            elseif _is_screen then
                                if ok_cs then CustomScreens.setIcon(_screen_id, new_icon) end
                            else
                                local c = Config.getCustomQAConfig(_id)
                            ...
                            _guardedSetIcon(new_icon, function(safe_icon)
                                ...
                                elseif _is_screen then
                                    -- No sentinel default icon needed here (unlike
                                    -- custom QAs' CUSTOM_ICON/etc.): a nil screen.icon
                                    -- already falls back to Config.ICON.custom in
                                    -- registerQuickAction(), so safe_icon == nil just
                                    -- means "reset to default".
                                    if ok_cs then CustomScreens.setIcon(_screen_id, safe_icon) end
                                else
                                    local c = Config.getCustomQAConfig(_id)
                                    local type_default
        ...
        if #defaults > 0 then
            rows[#rows + 1] = {
                ...
                sui_build = function(ctx)
                    return require("engines/sui_window").SectionLabel{ text = _("System Actions"):upper(), inner_w = ctx.inner_w }
                end
            }
        ...
        if #customs > 0 then
            rows[#rows + 1] = {
                ...
                is_divider = true,
                sui_build = function(ctx)
                    return require("engines/sui_window").SectionLabel{ text = _("Custom Actions"):upper(), inner_w = ctx.inner_w }
                end
            }
        ...
        if #screens > 0 then
            rows[#rows + 1] = {
                text = _("Custom Screens"):upper(),
                is_divider = true,
                sui_build = function(ctx)
                    return require("engines/sui_window").SectionLabel{ text = _("Custom Screens"):upper(), inner_w = ctx.inner_w }
                end
            }
            for _, entry in ipairs(screens) do add_entry(entry) end
        end
        return rows
    end
...
function QA.showQuickActionDialog(plugin, qa_id, on_done)
    ...
    local TOTAL_H     = require("screens/sui_bottombar").TOTAL_H
    ...
    -- Forward-declared alongside openActionPicker above: cancelActionPicker()
    -- (defined next) calls _buildSaveDialog() but is itself defined before
    -- _buildSaveDialog's own declaration further down. Without this forward
    -- declaration, that call would resolve to a nonexistent global and crash
    -- with "attempt to call global '_buildSaveDialog' (a nil value)" whenever
    -- cancelActionPicker() runs with a type or qa_id already set (i.e.
    -- cancelling any of the sub-pickers, not just the very first one).
    ...
    _buildSaveDialog = function(update_name_with_title)
        ...
        local function openIconPicker()
            ...
            QA.showIconPicker(chosen_icon, function(new_icon)
                ...
                -- When the icon picker is cancelled, go back to the properties
                _buildSaveDialog(false)
            end)
        end
...
-- ---------------------------------------------------------------------------
-- hasCustomIcons() — true when at least one Quick Action icon (built-in
-- action default, custom QA, Custom Screen, or a "sui_qa_defaults"
-- system-icon slot) has been overridden from its default. Single source of
-- truth for the "✎" indicator shown next to "Quick Action Icons" in both
-- Style → Icons and the Quick Actions screen.
-- ---------------------------------------------------------------------------
function QA.hasCustomIcons()
    ...
    if ok_cs and CustomScreens then
        for _, screen in ipairs(CustomScreens.list()) do
            if screen.icon ~= nil then return true end
        end
    end
    local ok_ss, SUIStyle = pcall(require, "features/sui_style")
    if ok_ss and SUIStyle then
    ...
    local function buildRoot(ctx)
        ...
        if mqa and mqa.buildQAWidget and mqa.getQADims and mqa.FRAME_SZ then
            ...
            if #valid_items > 0 then
                ...
                local ok_core, UI_local = pcall(require, "infra/sui_core")
...
local function _recentMakeCoverCell(SH, fp, bd, cw, ch)
    ...
    local cover = SH.getBookCover(fp, cw, ch)
