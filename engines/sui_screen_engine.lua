...
local function buildSectionLabel(text, w, mod_id, right_text, page_nav, landscape_factor, pfx)
    ...
    local key = table.concat({
        tostring(mod_id or ""), text, tostring(w), color_key,
...
function ScreenWidget:_applyModuleBackground(mod_id, widget, w)
    local label_text = mod_id == "clock" and self._clock_label or nil
    return applyModuleBackground(mod_id, widget, w, label_text, nil,
        label_text ~= nil, nil, nil, self._pfx)
end
...
function ScreenEngine._open(instance_cfg, on_qa_tap, on_goal_tap)
    ...
    closeStaleHomescreens(existing)
    if existing then
    ...
    local w = ScreenWidget:new{
        ...
        _force_full_repaint_once = _sget(id, "_force_full_repaint_once"),
    }
    _sset(id, "_force_full_repaint_once", nil)
